...
`dota2-mcp` — a Model Context Protocol server (stdio, or one shared HTTP instance) that gives an MCP
...
node dist/server.js --http   # SHARED mode: one process serves every client (see below)
...
By default every client spawns its own process, and each one builds its own copy of the
index + dependency graph — ~1&nbsp;GB resident apiece. With several Codex threads and a few
Claude Code sessions open that adds up fast.

`--http [port]` (or `MCP_HTTP_PORT`) instead serves **every** client from one process over
streamable HTTP on `127.0.0.1:7331`. The index, graph, API data and addon watcher live at
module scope and are shared; each session gets its own `McpServer` via `createMcpServer()`,
because a `Server` binds exactly one transport and concurrent clients would otherwise
collide on JSON-RPC request ids. `GET /health` reports index status and live session count.
...
```jsonc
// .mcp.json
{ "mcpServers": { "dota2-mcp": { "type": "http", "url": "http://127.0.0.1:7331/mcp" } } }
```
```toml
# .codex/config.toml
...
Two caveats. **Nothing respawns it:** unlike a stdio server, a client will not start this
process — if it is down the tools simply show as unavailable, so install the scheduled task.
And **the active addon cannot come from cwd** any more (one process, many clients), so it is
pinned via `ADDON_PATH` in the launcher; `reindex { "addonPath": "..." }` re-points it at
runtime — globally, for every connected client.
...
In stdio mode stdout carries the protocol stream, so **all logging must
...
```
L4  src/server.ts          — registers 29 MCP tools (stdio or shared HTTP); registerTool is wrapped
